...
class WeGroupNavbar extends Component {
    ...
    render() {
        return (
            <Navbar className='top-nav' bg="danger" expand="lg">
                <Navbar.Brand style={{color: 'white', fontSize: '22px'}} href="/">WeGroup</Navbar.Brand>
                <Navbar.Toggle aria-controls="basic-navbar-nav" />
                <Navbar.Collapse id="basic-navbar-nav">
                    <Nav className="mr-auto">
                        {this.props.token ? <Nav.Link onClick={this.logoutUser}>Logout</Nav.Link> : <Nav.Link href="/login">Login</Nav.Link>}
                    </Nav>
                    {this.props.showSearch && <Tooltip title="Search Tag">
                        <IconButton aria-label="search">
                            <SearchIcon onClick={this.props.showSearch} style={{fontSize: '25px', color: 'white', margin: 'auto'}} />
                        </IconButton>
                    </Tooltip>}
                    {this.props.token && <Nav.Link style={{marginRight: '2rem'}} href="/create_post"><Tooltip title="Create Post">
                        <IconButton aria-label="post">
                            <CreateIcon style={{fontSize: '25px', color: 'white', margin: 'auto'}} />
                        </IconButton>
                    </Tooltip></Nav.Link>}
                    {this.props.token && <Nav.Link href="/profile" style={{padding: 0}}>
